...
export const Dashboard = async () => {
    ...
    let modulos: string[] = ['venda']; // Default
    let catalogMetrics: any = null;

    if (user?.role === 'admin') {
        modulos = ['catalogo', 'venda', 'agendamento', 'disparo']; // Admin sees all
        try {
        ...
        try {
            ...
            const company = companyDoc as any;
            modulos = company?.modulos_ativos || ['venda'];

            // Count messages generated by IA (venda ou agendamento usam mensagens)
            if (modulos.includes('venda') || modulos.includes('agendamento')) {
            ...
            if (modulos.includes('venda') || modulos.includes('catalogo')) {
